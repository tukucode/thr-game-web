...
onMounted(() => {
  ...
  console.group('🔍 PWA Detection Diagnostics');
...
<template>
  <div v-if="showPrompt" class="fixed bottom-6 left-1/2 -translate-x-1/2 z-[100] w-[92%] max-w-md px-2 pointer-events-none">
    <div 
      ref="promptRef"
    ...
    >
      ...
      <div class="flex items-start gap-4 z-10">
        <div class="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center shrink-0 border border-primary/20 shadow-inner">
          <img src="/source/ketupat.png" alt="App Icon" class="w-10 h-10 object-contain drop-shadow-md p-1" />
        </div>
        
        <div class="flex-1">
          <h3 class="font-extrabold text-lg text-foreground leading-tight mb-1.5 tracking-tight">
      ...
      <div v-if="isAppleDevice" class="bg-primary/5 rounded-2xl p-4 border border-primary/10 flex flex-col gap-3 z-10">
        <div class="flex items-center gap-3 text-xs font-bold text-primary/80">
          <Icon icon="lucide:info" class="w-4 h-4" />
          CARA INSTAL MANUAL:
        </div>
        <div class="flex flex-col gap-2.5">
          <div class="flex items-center gap-4 text-xs font-semibold text-foreground/80">
            <span class="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center text-[10px] text-primary shrink-0">1</span>
            <span>Tap tombol <Icon icon="lucide:share" class="inline-block w-4 h-4 mx-1 text-primary" /> <b>'Share'</b> di browser kamu</span>
          </div>
          <div class="flex items-center gap-4 text-xs font-semibold text-foreground/80">
            <span class="w-6 h-6 rounded-full bg-primary/20 flex items-center justify-center text-[10px] text-primary shrink-0">2</span>
            <span>Pilih <b>'Add to Home Screen'</b> atau <b>'Add to Dock'</b> <Icon icon="lucide:plus-square" class="inline-block w-4 h-4 mx-1 text-primary" /></span>
          </div>
        </div>
      </div>
