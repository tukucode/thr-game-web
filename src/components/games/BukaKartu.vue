...
const generateOtherValues = () => {
  const masterList = config.value.nominals.length > 0 ? config.value.nominals : [0]
  const values = []
    ...
    <div class="grid grid-cols-3 gap-3 sm:gap-4 w-full p-5 mb-4 bg-primary/5 rounded-3xl border border-primary/10 shadow-inner">
      <button 
        ...
        :class="[
          ...
          store.isRevealed && i - 1 !== selectedIndex ? 'opacity-40 grayscale' : '',
          store.isRevealed && i - 1 === selectedIndex ? 'z-10 ring-4 ring-primary scale-105' : ''
        ]"
      >
        <div 
          class="w-full h-full transition-transform duration-700 transform-style-3d relative"
          :class="{ 'rotate-y-180': store.isRevealed && (i - 1 === selectedIndex) || (selectedIndex === i - 1 && !store.isRevealed) }"
        >
          <!-- Bagian Belakang Kartu (Motif) -->
          <div class="absolute inset-0 w-full h-full backface-hidden rounded-xl bg-gradient-to-br from-primary to-primary/80 border-2 border-primary-foreground/20 flex flex-col items-center justify-center p-2 shadow-inner">
          ...
          <div class="absolute inset-0 w-full h-full backface-hidden rotate-y-180 rounded-xl bg-card border flex items-center justify-center shadow-md p-2 sm:p-3"
               :class="i - 1 === selectedIndex ? 'border-primary bg-primary/10 dark:bg-primary/5 ring-inset ring-2 ring-primary/50' : 'border-border bg-accent/20'">
            
            <div class="flex flex-col items-center gap-2 sm:gap-1 text-center w-full">
              <!-- Header untuk pemenang -->
              <template v-if="i - 1 === selectedIndex">
                <Icon icon="lucide:party-popper" class="w-8 h-8 sm:w-10 sm:h-10 text-primary animate-bounce filter drop-shadow-sm" style="animation-duration: 2s" />
                <div class="px-1 py-1 sm:px-2 sm:py-1.5 bg-primary/10 dark:bg-primary/20 rounded-md w-full border border-primary/20">
                  <span class="text-[0.7rem] sm:text-xs block text-foreground dark:text-primary uppercase tracking-widest mb-0.5">Selamat!</span>
                  <span class="text-xs sm:text-sm font-bold text-foreground dark:text-white leading-tight block truncate">
              ...
              <template v-else>
                <div class="flex flex-col items-center gap-1 sm:gap-2 opacity-60">
                   <div class="text-[0.75rem] sm:text-xs font-bold text-muted-foreground uppercase tracking-wider mb-1">Nominal</div>
                   <span class="text-xs sm:text-sm font-bold text-foreground/80 leading-tight block truncate">
                      {{ formatRupiah(cardValues[i-1]) }}
                   </span>
                   <Icon icon="lucide:banknote" class="w-4 h-4 sm:w-6 sm:h-6 text-muted-foreground/40 mt-1" />
                </div>
              </template>
            </div>
          </div>
